...
def build_import_object(target_module: str, data: dict[str, Any]) -> Any:
    ...
    if target_module == "orders":
        ...
        order = Order(
            ...
            assigned_to_id=_as_int(data.get("assigned_to_id"), 0) or None,
            status=data.get("status") or "لم تبدأ",
            priority=data.get("priority") or "عادية",
            current_location=data.get("current_location") or "لم تحدد",
